...
options(lime_api="http://carceraria.limequery.com/admin/remotecontrol")
options(lime_username="jjesusfilho")
options(lime_password=rstudioapi::askForPassword("Please enter your password"))
get_session_key()

data <- get_responses(374445,
                           sLanguageCode = "pt-BR",
                           sHeadingType = "code"
...
nomes<-names(get_responses(374445,
                     sLanguageCode = "pt-BR",
                     sHeadingType = "full"
))
...
data2<- lapply(data,function(x) gsub("Nem discordo, nem concordo","Não discordo, nem concordo",x))
data2<-as.data.frame(data2) # transformou-se em lista, retornar a dataframe.
data2$id<-c(1:73) #corrigir a variável id para colocar em formato longo


data2<-melt(data2,id="id") #
datalikert<-data2[which(data2$value=="Discordo"| data2$value=="Mais discordo que concordo"| data2$value=="Discordo totalmente"|data2$value=="Não discordo, nem concordo"|data2$value=="Concordo"|data2$value=="Mais concordo que discordo"|data2$value=="Concordo totalmente"),]
datalikert<-cast(datalikert)
data.likert<-as.data.frame(datalikert)
...
## O procedimento abaixo cria sete níveis para os fatores

## Isto é só para efeito de plotagem likert
